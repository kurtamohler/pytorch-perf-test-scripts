...
print()
print('in_dtype out_dtype device order input-size method time-seconds')
for in_dtype, out_dtype in [(torch.float, torch.cfloat), (torch.double, torch.cdouble)]:
    for device in ['cpu', 'cuda']:
        for order in [1, 2, 3]:
            for input_size in [3, 8_000, 16_000, 44_100, 32 * 44_100]:
                input = torch.randn(input_size, dtype=in_dtype, device=device)
                output = torch.empty((), dtype=out_dtype, device=device)
                results = []
                for method_name, f in methods.items():
                    ...
                    print(f'{in_dtype} {out_dtype} {device} {order} {input_size} {method_name} {time_seconds}')
